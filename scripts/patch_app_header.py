#!/usr/bin/env python3
"""
Patch Application Binary with Size and CRC32
...
4. Patches the size and CRC32 into the header at offset 8 and 12
5. Writes the patched binary
...
Application header structure (32 bytes at offset 0):
  ...
  Offset 8:  size               - 4 bytes (PATCHED - size from 0x100 to end)
  Offset 12: crc32              - 4 bytes (PATCHED - CRC from 0x100 to end)
...
def patch_binary(input_file, output_file):
    """Patch binary with size and CRC32"""
    ...
    # Create patched header
    patched_header = bytearray(binary_data[:HEADER_SIZE])
    struct.pack_into('<I', patched_header, 8, firmware_size)   # Patch size
    struct.pack_into('<I', patched_header, 12, crc32)          # Patch CRC32
    ...
    output_data = bytes(patched_header) + padding + firmware_data
    ...
    # Print summary
    print("=" * 60)
    print("Application Binary Patching Complete")
    print("=" * 60)
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 patch_app_header.py <input.bin> <output.bin>")
        print("")
        print("This script patches the application header with:")
        ...
        print("The patched binary can then be uploaded via DFU.")
        return 1
    
    input_file = sys.argv[1]
    output_file = sys.argv[2]
    
    return patch_binary(input_file, output_file)
